...
class PDFAnnotationLinker:
    def __init__(self, pdf_path: str, mm_path: str, mode: str='text', image_size: float=0.5):
        self.mm_base, self.mm_name = os.path.split(path_Windows_to_Linux(mm_path))
        self.pdf_path = path_Linux_to_Windows(pdf_path)
        ...
        # 打开PDF和mm
        self.pdf = fitz.open(path_Windows_to_Linux(pdf_path))
    ...
    def parse_highlight(self, annot, wordlist) -> str:
        words = []
    ...
    def link_full_endpoint(self, endpoint: etree._Element, page_num: int, annot_num: int) -> None:
        print(f"P{page_num}-{annot_num}")
        method_map = {"text": self.change_text, "image": self.add_image}

        page = self.pdf[page_num]
        command = self.generate_command(page_num)
    ...
    def change_text(self, page, annot, endpoint: etree._Element) -> None:
        ...
        highlight = error_correction(self.parse_highlight(annot, wordlist))
    ...
    def generate_command(self, page_num: int) -> str:
        return command_template[platform].replace('PAGE_NUM', str(page_num)).replace("PDF_PATH", self.pdf_path)
...
if __name__ == '__main__':
    ...
    p = PDFAnnotationLinker(pdf_path, mm_path, mode, image_size)
    p.link_endpoints()
    p.save()
